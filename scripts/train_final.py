...
Purpose:
  Train the final selected fully connected neural network on a subset of BloodMNIST,
  using dropout and learning-rate decay, and evaluate it on both validation and test data.

What the script does:
  - Loads BloodMNIST train/val/test splits
  - Subsamples the train and validation sets (5000 train, 500 val)
  - Flattens and standardizes images using train mean/std
  - Trains a fully connected network with:
    layers         = [input_dim, 512, 256, 8]
    loss           = softmax
    optimizer      = Adam
    reg_strength   = 3e-4
    dropout_rate   = 0.2
    epochs         = 21
    lr             = 5e-4 (with step decay)
  - Plots:
  ...
  - Evaluates on the test split and prints:
    ...
    * Test accuracy
  - Computes and saves a confusion matrix on the test set.
"""
...
# --- Subsample train/val for faster training ---
...
# --- Reshape & standardize (use only train mean/std!) ---
...
# --- Model (best configuration from grid search) ---
